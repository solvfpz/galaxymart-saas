...
import { connectDB, getMongoDiagnostics } from '@/lib/mongodb';
...
export async function POST(req: Request) {
  ...
  try {
    try {
      ...
      const diag = getMongoDiagnostics(dbError);
      console.error(diag);
      return NextResponse.json(
        { success: false, message: "Database connection failed. Check server logs for details." },
        { status: 503 }
      );
    }
